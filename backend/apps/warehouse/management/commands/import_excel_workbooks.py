...
class Command(BaseCommand):
    ...
    def compute_health_scores(self) -> None:
        ...
        for company in Company.objects.all():
            ...
            profitability_score = self.clamp(self.float_value(latest_profit.opm_pct if latest_profit else None) * 4)
            growth_score = self.clamp(50 + (self.float_value(analysis_3y.compounded_sales_growth_pct if analysis_3y else None) * 2))
            leverage_score = self.leverage_score(latest_balance.debt_to_equity if latest_balance else None)
            cashflow_score = self.clamp(self.float_value(latest_cash.cash_conversion_ratio if latest_cash else None) * 70)
            dividend_score = self.clamp(self.float_value(latest_profit.dividend_payout_pct if latest_profit else None) * 1.5)
            trend_score = 70 if latest_profit and latest_profit.net_profit and latest_profit.net_profit > 0 else 35
